...
from utils.visualCNN.misc_functions import get_example_params, recreate_image, save_image
...
def execute(model, layerID, inputs, image_size=512):
    try:
        inverted_representation = InvertedRepresentation(model)
        inverted_representation.generate_inverted_image_specific_layer(inputs,
                                                                    image_size,
                                                                    layerID)
    except Exception as e:
        print("Couldn't execute: inverted_representation "+str(e))
