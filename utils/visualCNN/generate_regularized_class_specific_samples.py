...
class RegularizedClassSpecificImageGeneration():
    ...
    def __init__(self, model, target_class, output_path=None, in_shape=(224, 224, 3), mean=[-0.485, -0.456, -0.406], std=[1/0.229, 1/0.224, 1/0.225]):
        ...
        if output_path is None:
            ...
            self.output_path = os.path.join(output_path, 'RegularizedClassSpecificImageGeneration', 'class_'+str(self.target_class))
    ...
    def generate(self, iterations=150, blur_freq=4, blur_rad=1, wd=0.0001, clipping_value=0.1):
        ...
        for i in range(1, iterations):
            ...
            if i in np.linspace(0, iterations, 10, dtype=int):
                # Save image
                im_path = os.path.join(self.output_path, f'c_{self.target_class}_iter_{i}_loss_{class_loss.data.cpu().numpy()}.jpg')
        ...
        with open(os.path.join(self.output_path, 'run_details.txt'), 'w') as f:
...
def execute(model, classID, outpath=None, iterations=150, in_shape=(512,512,3)):
    try:
        csig = RegularizedClassSpecificImageGeneration(model, classID, outpath, in_shape)
