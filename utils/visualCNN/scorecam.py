...
from utils.visualCNN.misc_functions import get_example_params, save_class_activation_images
...
def execute(model, layerID, inputs, classID, file_name_to_export):
    try:
        # Score cam
        score_cam = ScoreCam(model, target_layer=layerID)
        # Generate cam mask
        cam = score_cam.generate_cam(inputs, classID)
        # Save mask
        save_class_activation_images(inputs, cam, file_name_to_export)
        print('Score cam completed')
    except Exception as e:
        print("Couldn't execute: score_cam "+str(e))
